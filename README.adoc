# Apache Artemis broker Kubernetes container image

NOTE: Extends link:https://github.com/arkmq-org/arkmq-org-broker-image[Apache Artemis broker container image]
...
$ oc process -n broker -f templates/apache-artemis-basic.yaml -p APPLICATION_NAME=broker-app -p AMQ_USER=<username> -p AMQ_PASSWORD=<amqpassword>  | oc create -n broker -f -
```
...
The Apache Artemis broker container images are pushed to https://quay.io/repository/arkmq-org/arkmq-org-broker-kubernetes

# Tags

The Apache Artemis broker container images have release tags and special tags.
